  ...
  return {
    tools: [
      ...
      },
      {
        ...
        description: "Create a new document in IT Glue for an organization. If neither document_folder_id nor skip_folder_prompt is supplied and the client supports elicitation, the user is prompted to pick a folder. When a JWT credential is configured the prompt is a name-based picker (preferred UX); otherwise it accepts a folder URL, a sibling-document URL, or a numeric folder ID (since folder names are not in API-key scope). Pass skip_folder_prompt=true to always create at the organization root without prompting.",
        inputSchema: {
          type: "object",
          properties: {
            organization_id: {
          ...
          if (haveJwt) {
            ...
            if (jwtClient) {
              try {
                ...
                // 401 invalidates the cached JWT and falls through to the URL parser;
                // any other error propagates because something is genuinely wrong.
                const msg = err instanceof Error ? err.message : String(err);
                if (msg.includes("401")) {
                  sessionJwt = undefined;
                } else {
